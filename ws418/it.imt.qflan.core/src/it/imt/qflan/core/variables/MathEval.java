...
//
// Contributions by Carlos Gómez of Asturias, Spain, in the area of unary operators
 ...
 *     '×'       40 / 40     NO_SIDE        Multiple (because it's a Unicode world out there)
...
extends Object
{
	...
	//ANDREA: MathEval does not support variales/constants containing the belowCharacters.
	//If you modify this method, please also check CRNImporter.loadViews 
	public static String getCorrespondingStringSupportedByMathEval(String string){
		return "s"+string.replace('~', 'T').replace(',', 'C').replace("(", "PO").replace(")", "PC").replace("_", "U").replace("!", "QM").replace(".", "D").replace("::", "CC").replace("$", "us").replace("@", "at").replace(" ", "").replace("+", "p").replace("*", "");
	}
	
	/*public static String getCorrespondingStringSupportedByMathEval(String string, int id){
		return "s"+id;
	}*/
	...
	private double _evaluate(int beg, int end, double lft, Operator pnd, Operator cur) throws NumberFormatException, ArithmeticException {
		...
		for(ofs=beg; (ofs=skipWhitespace(expression,ofs,end))<=end; ofs++) {
			...
			//ANDREA: modified to handle also numbers in this form: 56e-06, 56e+06, 56E-06, 56E+06     
			char prec = '_';
			char precPrec = '_';
			for(beg=ofs; ofs<=end; ofs++) {
				char chr=expression.charAt(ofs);
				//ANDREA: This is an heuristic to understand if we are dealing with a number in this form: 56e-06 or 56e+06 or 56E-06 or 56E+06 
				if((chr =='-' || chr =='+') && (prec == 'e' || prec == 'E') && Character.isDigit(precPrec) && (ofs + 1) <=end && Character.isDigit(expression.charAt(ofs + 1))){
					//I assume that we are dealing with a number in this form: 56e-06, 56e+06, 56E-06, 56E+06. I skip to the next iteration
					precPrec = prec;
					prec = chr;
					continue;
				}
	...
	implements OperatorHandler, FunctionHandler
	{
        ...
        switch(opr) {
            ...
            case '×' : return lft*rgt;                                                              // multiply (because it's a Unicode world out there)
		...
		public double evaluateFunction(String fncnam, ArgParser fncargs) throws ArithmeticException {
			...
			case 'l': {
				//ANDREA: I HAVE ADDED THIS CASE
				if(fncnam.equalsIgnoreCase("ln"           )) { return Math.log        (fncargs.next());                }
    ...
    static private final Operator       OPR_MLT2=new Operator('×',40                    ,DefaultImpl.INSTANCE); // multiply (because it's a Unicode world out there)
    ...
    static private final Operator       OPR_SUB =new Operator('-',20                    ,DefaultImpl.INSTANCE); // subtract/unary-negative
		...
		static void registerFunctions(MathEval tgt) {
			...
			//ANDREA: I have added this case
			tgt.setFunctionHandler("ln"           ,INSTANCE);
